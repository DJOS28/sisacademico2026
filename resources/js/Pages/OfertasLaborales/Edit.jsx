...
export default function Edit({ oferta, empresas = [], tiposContrato = [], planesEstudio = [] }) {
    // Función auxiliar para asegurar el formato YYYY-MM-DD necesario en input type="date"
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        id_tipo_contrato: oferta.id_tipo_contrato || '',
        id_plan_estudio: oferta.id_plan_estudio || '',
        titulo: oferta.titulo || '',
        descripcion: oferta.descripcion || '',
        fecha_publicacion: formatDate(oferta.fecha_publicacion),
        fecha_limite: formatDate(oferta.fecha_limite),
        lugar: oferta.lugar || '',
        modalidad: oferta.modalidad || 'Presencial',
        tipo_oferta: oferta.tipo_oferta || 'Empleo',
        remuneracion: oferta.remuneracion || '',
        vacantes: oferta.vacantes || 1,
        experiencia: oferta.experiencia || '',
        pasos_postular: oferta.pasos_postular || '',
        estado: oferta.estado || 'Publicada',
        archivo_pdf: null,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        // Usamos post con _method: 'PUT' para enviar archivos (PDF) correctamente mediante FormData
        post(route('ofertas-laborales.update', oferta.id_oferta), {
            forceFormData: true,
            onSuccess: () => {
                Swal.fire({
                    title: '¡Actualizado!',
                    text: 'La oferta laboral fue actualizada correctamente.',
                    icon: 'success',
    ...
    return (
                ...
                <div className="flex justify-between items-center w-full">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                            Modifica la información relevante de la vacante #{oferta.id_oferta}.
                        </p>
                    </div>
                    <Link
        ...
        >
            <Head title={`Editar - ${oferta.titulo}`} />
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-5 w-full">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                placeholder="Ej. 1. Enviar CV actualizado al correo rrhh@empresa.com con asunto CV-ASISTENTE..."
                                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-[#315d7a]"
                            ></textarea>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-[#315d7a]"
                                />
                                {errors.fecha_publicacion && <span className="text-xs text-rose-500 mt-1 block font-medium">{errors.fecha_publicacion}</span>}
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-[#315d7a]"
                                />
                                {errors.fecha_limite && <span className="text-xs text-rose-500 mt-1 block font-medium">{errors.fecha_limite}</span>}
                            </div>
