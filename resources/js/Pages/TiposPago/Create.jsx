...
export default function Create({ admisiones = [] }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        nombre_titular: '',
        activo: true,
        admisiones_ids: [], // Array de IDs para la tabla pivote admisiones_tipo_pago
    });

    // Manejo de checkboxes para la selección múltiple de admisiones
    ...
    // Seleccionar o desseleccionar todas las admisiones a la vez
    ...
    const guardar = (e) => {
        ...
        post(route('tipos-pago.store'), {
            ...
            onSuccess: () => {
                Swal.fire({
                    title: '¡Correcto!',
                    text: 'El medio de pago fue registrado y vinculado correctamente.',
                    icon: 'success',
    ...
    return (
        <AuthenticatedLayout header={<h1 className="text-2xl font-bold text-slate-900">Nuevo Medio de Pago</h1>}>
            ...
            <div className="w-full space-y-6">
                <div className="flex flex-col justify-between gap-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
                    <div>
                        <h2 className="text-lg font-bold text-slate-800">Registrar Medio de Pago</h2>
                        <p className="mt-1 text-sm text-slate-500">Complete los datos bancarios y asígnelo a los procesos de admisión correspondiente.</p>
                    </div>
                ...
                <form onSubmit={guardar} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Ej: Depósito BCP / Yape"
                            ...
                            <div className="md:col-span-2">
                                <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500">Estará activo en el sistema para poder recibir transacciones.</p>
                                    </div>
                                </label>
                            </div>
                        </div>
                    ...
                    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                            <div>
                                ...
                                <p className="text-xs text-slate-500">Marque las admisiones donde se mostrará esta cuenta bancaria a los postulantes.</p>
                            </div>
                            {admisiones.length > 0 && (
                                <button
                                    type="button"
                                    onClick={handleSelectAllAdmisiones}
                                    className="text-xs font-bold text-[#315d7a] hover:underline"
                                >
                    ...
                    <div className="flex flex-col-reverse justify-end gap-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row">
                        ...
                        <button type="submit" disabled={processing} className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#315d7a] px-5 py-2.5 text-sm font-bold text-white shadow transition hover:bg-[#274b63] disabled:opacity-60 cursor-pointer">
                            {processing ? 'Guardando...' : 'Guardar Medio de Pago'}
                        </button>
                    </div>
                </form>
            </div>
