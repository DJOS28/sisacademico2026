...
export default function Index({ tiposPago, filtros, resumen }) {
    const cambiarEstado = (tipo) => {
        router.patch(
            route('tipos-pago.cambiar-estado', tipo.id_tipo_pago),
            { activo: !tipo.activo },
            {
                preserveScroll: true,
    ...
    const eliminarTipoPago = (tipo) => {
        Swal.fire({
            title: '¿Eliminar medio de pago?',
            text: `Se eliminará "${tipo.nombre}". Esta acción no se puede deshacer.`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                router.delete(route('tipos-pago.destroy', tipo.id_tipo_pago), {
                    preserveScroll: true,
                    onSuccess: () => Swal.fire('Eliminado', 'El medio de pago fue eliminado.', 'success'),
    ...
    return (
        <AuthenticatedLayout header={<h1 className="text-2xl font-bold text-slate-900">Medios de Pago</h1>}>
            ...
            <div className="w-full space-y-6">
                ...
                <div className="flex flex-col justify-between gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
                    <div>
                        <h2 className="text-lg font-bold text-slate-800">Listado de Medios de Pago</h2>
                        <p className="mt-1 text-sm text-slate-500">Cuentas bancarias y métodos habilitados para admisión.</p>
                    </div>

                    <Link
                        href={route('tipos-pago.create')}
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div className="rounded-xl border border-slate-200 border-l-4 border-l-slate-600 bg-white p-4 shadow-sm">
                        <p className="text-xs font-semibold uppercase text-slate-500">Total Registrados</p>
                    ...
                    <table className="w-full text-left text-xs">
                        <thead className="border-b border-slate-200 bg-slate-50 font-semibold text-slate-700">
                            <tr>
                                ...
                                <th className="px-4 py-3">Banco / Entidad</th>
                                <th className="px-4 py-3">N° Cuenta / CCI</th>
                        ...
                        <tbody className="divide-y divide-slate-100 text-slate-700">
                            {tiposPago.data && tiposPago.data.length > 0 ? (
                                tiposPago.data.map((tp) => (
                                    <tr key={tp.id_tipo_pago} className="hover:bg-slate-50">
                                        <td className="px-4 py-3 font-semibold text-slate-800">{tp.nombre}</td>
                                        <td className="px-4 py-3">{tp.banco_o_entidad}</td>
                                        <td className="px-4 py-3 font-mono text-[11px]">
                                            <div>Cta: {tp.numero_cuenta}</div>
                                            <div className="text-slate-400">CCI: {tp.cci}</div>
                                        </td>
                                        <td className="px-4 py-3">{tp.nombre_titular}</td>
                                        <td className="px-4 py-3 text-center">
                                            <button
                                                type="button"
                                                onClick={() => cambiarEstado(tp)}
                                                className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold cursor-pointer ${
                                                    tp.activo ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'
                                                }`}
                                            >
                                        ...
                                        <td className="px-4 py-3 text-right space-x-2">
                                            <Link
                                                href={route('tipos-pago.edit', tp.id_tipo_pago)}
