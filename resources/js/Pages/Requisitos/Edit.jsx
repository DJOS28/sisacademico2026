...
export default function Edit({ requisito }) {
    const { data, setData, put, processing, errors } = useForm({
        nombre: requisito.nombre || '',
        descripcion: requisito.descripcion || '',
        activo: Boolean(requisito.activo),
    });

    const actualizar = (e) => {
        e.preventDefault();
        put(route('requisitos.update', requisito.id_requisito), {
            preserveScroll: true,
            onSuccess: () => {
                Swal.fire({
                    title: '¡Actualizado!',
    ...
    return (
        <AuthenticatedLayout header={<h1 className="text-2xl font-bold text-slate-900">Editar Requisito</h1>}>
            <Head title={`Editar - ${requisito.nombre}`} />
            ...
            <div className="w-full space-y-6">
                <div className="flex flex-col justify-between gap-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
                    <div>
                        <h2 className="text-lg font-bold text-slate-800">Modificar Requisito</h2>
                        <p className="mt-1 text-sm text-slate-500">Actualice la información del requisito.</p>
                    </div>
