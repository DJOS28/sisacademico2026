...
export default function Create({ requisitosDisponibles = [] }) {
    ...
    // Filtrado de requisitos en tiempo real
    ...
    const guardar = async (event) => {
        ...
        try {
            const { data } = await axios.post(route('tramites.store'), form);

            await Swal.fire({
                icon: 'success',
                title: 'Trámite registrado',
                text: data.message ?? 'El trámite y sus requisitos se guardaron con éxito.',
                timer: 2000,
        ...
        <AuthenticatedLayout
            header={
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-xl font-bold text-slate-900 tracking-tight">Nuevo Trámite Institucional</h1>
                        <p className="text-xs text-slate-500">Configuración general y asignación de requisitos documentarios.</p>
                    </div>
                    <Link
        ...
        >
            ...
            <form onSubmit={guardar} className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    <div className="lg:col-span-5 space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
                        <div className="border-b border-slate-100 pb-3">
                            ...
                            <p className="text-xs text-slate-500">Información pública y arancelaria del trámite.</p>
                        </div>
                    ...
                    <div className="lg:col-span-7 space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
                            <div>
                                ...
                                <p className="text-xs text-slate-500">Documentos y condiciones que el solicitante debe presentar.</p>
                            </div>
                            <span className="bg-[#315d7a]/10 text-[#315d7a] font-bold text-xs px-3 py-1 rounded-full border border-[#315d7a]/20 self-start">
                                {form.requisitos.length} seleccionados
                            </span>
                        </div>
                ...
                <div className="flex justify-end gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-xs">
                    ...
                    >
                        {guardando ? 'Guardando trámite...' : 'Guardar Trámite Completo'}
                    </button>
                </div>
            </form>
        </AuthenticatedLayout>
