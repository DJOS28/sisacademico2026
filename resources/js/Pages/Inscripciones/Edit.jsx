...
export default function Edit({ inscripcion, admisiones = [], postulantes = [], planesEstudio = [] }) {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (busqueda.trim() !== '') {
                router.get(
                    route('inscripciones.edit', inscripcion.id_inscripcion),
    ...
    const actualizar = (e) => {
        ...
        put(route('inscripciones.update', inscripcion.id_inscripcion), {
            preserveScroll: true,
            onSuccess: () => {
                Swal.fire('¡Actualizado!', 'Inscripción actualizada con éxito.', 'success');
            },
            onError: () => {
                Swal.fire('Error', 'Revise la información.', 'warning');
            },
        });
    };
        ...
        <AuthenticatedLayout header={<h1 className="text-2xl font-bold text-slate-900">Editar Inscripción</h1>}>
            ...
            <div className="w-full space-y-6">
                <div className="flex flex-col justify-between gap-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
                    <div>
                        <h2 className="text-lg font-bold text-slate-800">Modificar Inscripción</h2>
                        <p className="mt-1 text-sm text-slate-500">Actualice la información del registro de inscripción.</p>
                    </div>
                ...
                <form onSubmit={actualizar} className="space-y-6">
                    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                            ...
                            <div>
                                <label className="mb-1 block text-xs font-semibold text-slate-700">Segunda Opción</label>
                                <input
                                    type="text"
